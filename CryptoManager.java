...
Summary of Description: Write a Java program to encrypt and decrypt a phrase using two
similar approaches, each insecure by modern standards(Caesar and Bellaso Cipher).
Due Date: 03/06/2023
Integrity Pledge: I pledge that I have completed the programming assignment independently.
I have not copied the code from a student or any source.
Student: Lawrence Aryeh
 */
...
public class CryptoManager {
	...
	public static String bellasoDecryption(String encryptedText, String bellasoStr) {
		String decrypted = "";
		char characterDecrypted;
		char characterKeyWord;
		int key;



		if (bellasoStr.length() < encryptedText.length()) {//if key word length is less than original text

			int difference = encryptedText.length() - bellasoStr.length(); // length difference
		...
		for (int i = 0; i < encryptedText.length(); i++) { // iterate through the original text string

			characterDecrypted = encryptedText.charAt(i); // assign the current character of the original text
			characterKeyWord = bellasoStr.charAt(i); // assign the current character of  the keyword

			int charEncrypted = (int) characterDecrypted; //ascii integer of CharOrigin
			int charKey = (int) (characterKeyWord); //ascii integer of charKeyWord

			key = charEncrypted - charKey; // add both integer of chars[charOrigin and charKey]

			while (key < LOWER_RANGE) { // if key is greater than range and less than Lower range
				key += RANGE;
			}


			characterDecrypted = (char) (key); // convert the corresponding char of the integer key

			decrypted += characterDecrypted; // add the encrypted letter
